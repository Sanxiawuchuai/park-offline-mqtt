...
public class yunEventImpl implements YunEventSever {
	...
	public void scanCodesInOut(String jsonStr) {
		...
		MainBoardMessage<YunScanCodeHead, YunScanCodeInOutBody> messgeVo = JsonUtil.jsonToBoardMessage(jsonStr,
				YunScanCodeHead.class, YunScanCodeInOutBody.class);
		YunScanCodeInOutBody model = messgeVo.getBody();
		YunScanCodeInOutReplayHead yunHead = new YunScanCodeInOutReplayHead();
		try {
			logger.debug("出入口扫码=》" + jsonStr);
			ParkChannelSet channel = ParkMethod.getChannelSetByEquipmentID(model.getDsn());
			if (channel != null) {
				...
				if (in != null && in.getFrmType() == 1) {
					...
					if (local != null && local.getOnline() == 1) {
						boxInIsOpen(local.getEquipmentID(), channel.getChannelIp());
					}
					...
					yunHead.setStatus(1);
					YunScanCode<YunScanCodeInOutReplayHead, YunScanCodeInModel> retInfo = new YunScanCode<>(yunHead,
							body);
					String json = JsonUtil.objectToJsonStr(retInfo);
					logger.debug("入口扫码返回=》" + json);
					...
					if ((int) isSuccess == 0) {
						...
						pushBackChargeData(out, messgeVo.getHead().getReplyTopic(), model.getDsn());
			...
			logger.error("scanCodesInOut=》" + ex);
		}
	...
	private void sendToYunChageData(YunScanCodeInOutReplayHead yunHead, String topic) {
		yunHead.setStatus(0);
		YunScanCodeInModel body = new YunScanCodeInModel();
		YunScanCode<YunScanCodeInOutReplayHead, YunScanCodeInModel> retInfo = new YunScanCode<>(yunHead, body);
		String json = JsonUtil.objectToJsonStr(retInfo);
		logger.debug("出入口扫码返回=>" + json);
		ofMqttService.sendMessage(topic, json, null, 0);
	}

	/** 推送入场 */
	public void boxInIsOpen(String equipmentID, String channelIP) {
		...
		MqttMessageVO retVo = MqttServiceImpl.sendMessage(equipmentID, topic, jsonBody, null, 0);
	}
	...
	public void getCentrilChargeData(String jsonStr) {
		...
		try {
			...
			logger.error("getCentrilChargeData=》" + ex);
			yunHead.setStatus(0);// 失败
	...
	public void writeChargeData(String jsonStr) {
		MainBoardMessage<YunScanCodeHead, PayMentCarFee> messgeVo = JsonUtil.jsonToBoardMessage(jsonStr,
				YunScanCodeHead.class, PayMentCarFee.class);
		PayMentCarFee model = messgeVo.getBody();
		YunScanCodeInOutReplayHead yunHead = new YunScanCodeInOutReplayHead();
		try {
			logger.debug("下发缴费信息到线下=》" + jsonStr);

			if (model != null) {
				ParkCarIn in = parkCarInMapper.selectTop(model.getCarNo().substring(1), 0);
				if (in == null)
					return;
				Date t1 = new Date();
				saveCentrialChargeData(model, in,t1);
				if (!StringUtils.isNullOrEempty(model.getDsn()))// 出口扫码存在
				{
					ParkChannelSet channel = ParkMethod.getChannelSetByEquipmentID(model.getDsn());
					if (channel != null) {
						OutRealTimeBase out = GlobalPark.outChannelEvent.get(channel.getChannelIp());
						ParkCarOut parkout = getParkCarOurModel(in, out.getOut(), model, channel,t1);
						out.setOut(parkout);
						...
						parkingOutService.saveParkOutRecord(out);
						parkingOutService.sendParkCarOutRecordToBox(out);
						parkingOutService.open(channel.getDsn());
						parkingOutService.pushChargeData(out);
						out.setNextStep(Step.VOICE);
						parkingOutService.broadcastVoice(out);
						GlobalPark.outChannelEvent.remove(channel.getChannelIp());
			...
			logger.error("writeChargeData=》" + ex);
			yunHead.setStatus(0);
		} finally {
	...
	/** 保存中央收费记录 */
	private void saveCentrialChargeData(PayMentCarFee model, ParkCarIn in,Date t1) {
...
//			Vo.setPayChargeTime(StrToDate(model.getPayTime()));
		...
		{
			if (model.getPayTime() != null)
				Vo.setOverTime(StrToDate(model.getPayTime()));
		}
		parkCentralChargeMapper.insertSelective(Vo);
		parkingCentreService.centreStatisticsRefresh(Vo);
		if (!StringUtils.isNullOrEempty(boxIp))
			parkingCentreService.centreSumUser(Vo, boxIp);

	}
	

	private ParkCarOut getParkCarOurModel(ParkCarIn in, ParkCarOut out, PayMentCarFee model, ParkChannelSet channel,Date t1) {
	...
	public void onlineRenewal(String jsonStr) {
		MainBoardMessage<YunScanCodeHead, RenewalVO> messgeVo = JsonUtil.jsonToBoardMessage(jsonStr,
				YunScanCodeHead.class, RenewalVO.class);
		RenewalVO renewalVO = messgeVo.getBody(); // 得到Body的相关值信息
		YunScanCodeInOutReplayHead yunHead = new YunScanCodeInOutReplayHead();
		try {
			logger.debug("线上固定卡续费下发=》" + jsonStr);
			if (renewalVO != null) {
			...
			logger.error("线上续费下发异常：" + e.getMessage());
			yunHead.setStatus(0);
	 ...
	 * @param channelId
	 */
	private void pushBackChargeData(OutRealTimeBase out, String replayTopic, String dsn) {
		YunScanCodeInOutReplayHead yunHead = new YunScanCodeInOutReplayHead();
		yunHead.setStatus(1);
		...
		YunScanCode<YunScanCodeInOutReplayHead, YunScanCodeOutModel> retInfo = new YunScanCode<>(yunHead, body);
		String json = JsonUtil.objectToJsonStr(retInfo);
		logger.debug("出口扫码返回=》" + json);
